...
describe("programmatic API", () => {
	...
	describe("recordExpertise", () => {
		...
		it("throws on unknown domain", async () => {
			...
			await expect(recordExpertise("nonexistent", record, { cwd: tmpDir })).rejects.toThrow(
	...
	describe("editRecord", () => {
		...
		it("throws on unknown domain", async () => {
			await expect(editRecord("nonexistent", "mx-abc123", {}, { cwd: tmpDir })).rejects.toThrow(
				/Domain "nonexistent" not found/,
			);
		});

		it("throws on non-existent record ID", async () => {
			await expect(editRecord("testing", "mx-ffffff", {}, { cwd: tmpDir })).rejects.toThrow(
				/not found/,
			);
		});
