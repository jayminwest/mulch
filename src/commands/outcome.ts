...
export function registerOutcomeCommand(program: Command): void {
	program
		...
		.addOption(
			new Option("--status <status>", "outcome status").choices(["success", "failure", "partial"]),
		)
		...
		.action(async (domain: string, id: string, options: Record<string, unknown>) => {
			...
			try {
					...
					if (jsonMode) {
						...
						if (!isQuiet()) {
							if (outcomes.length === 0) {
								console.log(chalk.dim("No outcomes recorded for this record."));
							} else {
								console.log(
								...
								for (const [i, o] of outcomes.entries()) {
									...
									if (o.test_results) console.log(chalk.dim(`     tests: ${o.test_results}`));
									if (o.notes) console.log(chalk.dim(`     notes: ${o.notes}`));
									if (o.recorded_at) console.log(chalk.dim(`     recorded: ${o.recorded_at}`));
								}
							}
						}
					}
				...
				if (jsonMode) {
					outputJsonError("outcome", err instanceof Error ? err.message : String(err));
				} else {
