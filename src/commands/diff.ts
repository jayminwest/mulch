		...
		.action(async (options: { since: string }) => {
			...
			try {
				...
				for (const entry of entries) {
					const changeCount = entry.added.length + entry.removed.length;
					const plural = changeCount === 1 ? "change" : "changes";
					if (!isQuiet()) console.log(chalk.cyan(`${entry.domain} (${changeCount} ${plural}):`));

					for (const record of entry.added) {
						const summary = getRecordSummary(record);
						if (!isQuiet())
							console.log(
								chalk.green(`  + [${record.type}] ${accent(record.id ?? "")} ${summary}`),
							);
					}
					...
					for (const record of entry.removed) {
						const summary = getRecordSummary(record);
						if (!isQuiet())
							console.log(chalk.red(`  - [${record.type}] ${accent(record.id ?? "")} ${summary}`));
					}
