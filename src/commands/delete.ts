...
function printDeletedRecord(record: ExpertiseRecord, domain: string, dryRun: boolean): void {
...
export function registerDeleteCommand(program: Command): void {
	program
		...
		.option("--all-except <ids>", "delete all records except these comma-separated IDs")
		...
		.action(
			async (
				...
				try {
					...
					const modeCount = [hasId, hasRecords, hasAllExcept].filter(Boolean).length;

					if (modeCount === 0) {
						if (jsonMode) {
							outputJsonError("delete", "Must provide a record ID, --records, or --all-except.");
						} else {
							console.error(
								chalk.red("Error: must provide a record ID, --records, or --all-except."),
							);
						}
						process.exitCode = 1;
						return;
